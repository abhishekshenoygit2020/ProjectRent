function printInvoiceAction(request, response) {
    ...
    var salesOrdId = invoice.getFieldValue("custbody_rent_sales_order");
    ...
    template += '            <tr><td style="width:100%;"><i>A. Any discrepancies must be notified to Accounts Department within 7 days from receipt of this invoice</i></td></tr>';
    template += '            <tr><td style="width:100%;"><i>B. Invoice should be settled as per approved credit period. In this case of delay fee of 2% will be levied for every month</i></td></tr>';
    template += '          </table>';
    // if (footerlogobottom) {
    ...
    template += "</style> ";
    template += "</head>";
    template += "    	 <body header=\"nlheader\" header-height=\"5%\" footer=\"nlfooter\" footer-height=\"5%\" size=\"A4\"> ";




    template += "<table style=\"width:100%; padding-top:10px;font-size:-30px;font-weight:bold;\"> ";
    ...
    // template += " <td style=\"width:250px;font-weight:bold;\" align=\"left\">${record.custbody_bill_to_name}<td>";
    // template += " <#else>";
    ...
    template += " <td style=\"width:339px;font-weight:bold;font-size:12px;\" align=\"left\">CR No / <span class=\"arabicfont\" dir=\"rtl\">سجل تجاري </span> : ${record.entity.custentity_cr_number}</td>";
    template += " </tr>";
    ...
    // template += " <td style=\"width:359px;font-weight:bold;font-size:9px;\" align=\"left\"><span class=\"arabicfont\" dir = \"rtl\">${record.billaddress}</span></td>";
    // template += " <#else>";
    template += " <td style=\"width:359px;font-weight:bold;font-size:9px;\" align=\"left\"><span class=\"arabicfont\" dir = \"rtl\"></span></td>";
    // template += " </#if>";
    ...
    template += " </tr>";
    template += " <tr> ";
    template += " <td style=\"width:60px;font-weight:bold;font-size:12px;\" align=\"left\"></td>";
    template += " <td style=\"width:22px;font-weight:bold;font-size:12px;\" align=\"right\">";
    ...
    // ELSE PART-----STARTING HERE---------------------------------//

    // template += " <#else>";// else for 1st if
    // nlapiLogExecution("DEBUG", "inside if--=====--")
    ...
    // template += " <tr>";//fetch bill to
    // template += " <#if record.custbody_legal_entity?has_content>";//4 if
    // nlapiLogExecution("DEBUG", "inside hi raeched++++++++++++++++++++++++++++++====--")
    ...
    // template += " <td style=\"width:339px;font-weight:bold;\" align=\"left\">${record.entity.custentity_customer_name}<br/><span class=\"arabicfont\" dir =\"rtl\">${record.entity.custentity_customer_long_name_arabic}</span></td>";
    // template += " </#if>";
    // template += " </tr>";

    // template += " <tr> ";
    // template += " <td style=\"width:80px;\">&nbsp;</td>";
    // template += " </tr> ";


    // template += " <tr> ";
    // template += " <td style=\"width:359px;font-weight:bold;font-size:12px;\" align=\"left\">VAT No /<span class=\"arabicfont\" dir=\"rtl\">الرقم الضريبي</span> : ${record.entity.vatregnumber}</td>";
    // template += " </tr> ";
    ...
    // template += " <td style=\"width:48%; border-left:solid 1px #000000;\">";//right td
    // template += " <table style=\"width:100%\">";
    // template += " <tr>";
    // template += " <td style=\"width:60px;font-weight:bold;font-size:12px;margin-top:-18px;\" align=\"left\">Invoice No / <span class=\"arabicfont\" dir=\"rtl\">رقم الفاتورة</span> : </td>";
    // template += " <td style=\"width:42px;font-weight:bold;font-size:12px;margin-top:-18px;\" align=\"right\">${record.tranid}</td> ";
    // template += " </tr>";

    // template += " <tr>";
    // template += " <td style=\"width:60px;font-weight:bold;font-size:12px;margin-top:-35px;\" align=\"left\">Invoice Date / <span class=\"arabicfont\" dir=\"rtl\">تاريخ الفاتورة</span> : </td>";
    // template += " <td style=\"width:22px;font-weight:bold;font-size:12px;margin-top:-35px;\" align=\"right\">${record.trandate?string[\"d.MM.yyyy\"]}</td> ";
    // template += " </tr>";

    // template += " <tr> ";
    // template += " <td style=\"width:60px;font-weight:bold;font-size:12px;margin-top:-35px;\" align=\"left\">Alfaris VAT No /<br/><span class=\"arabicfont\" dir=\"rtl\"> الرقم التعريف الضريبي </span> : </td>";
    // template += " <td style=\"width:22px;font-weight:bold;font-size:12px;margin-top:-35px; \" align=\"right\">${record.custbody_trn}<br/>";
    // template += " <#if record.custbody_qr_code_data?has_content>";//5 if
    ...
    template += " </tr>";
    template += " <tr> ";
    template += " <td> <p style =\"min-height:20px;\"><b>Fax No / <span class=\"arabicfont \" dir= \"rtl \"> رقم الفاكس </span> : </b> <br/><#if record.entity.fax?has_content> ${record.entity.fax} <#else> &nbsp;</#if> </p></td> ";
    template += " </tr>";
    template += " <tr> ";
    ...
    template += "  <table style=\"width:100%\">";
    template += "  <tr>";
    template += " <td> <p style =\"min-height:20px;\"><b>Pay Terms / <span class=\"arabicfont\" dir=\"rtl\">وقت الدفع المسبق </span> :</b>  <br/> ${record.terms} </p></td>";
    template += " </tr>";
    ...
    // ----TESTING STARTS
        ...
        template += "  ";
        template += "      <table style=\"width:100%; margin-bottom: 0px; padding-top:-16px;\"> ";
        template += "        <tr><td style=\"width:20%; \" ><b>Scope of work <span class=\"arabicfont\" dir=\"rtl\">نوع العمل</span></b></td><td style=\"width:1%;\"><b>:</b></td><td style=\"width:74%;\">${record.custbody_scope_of_work}</td></tr>";
        // template += "</table> <table><tr><td></td></tr></table>";
        template += "</table> ";
        template += "    <table style=\"width:100%; padding-top:9px\"> ";
    ...
    if ("T" == invoice.getFieldValue("custbody_allow_lump_sum")) {
        ...
        if (lineCount > 0) {
            for (var i = 1; i <= lineCount; i++) {
                ...
                ///////////////////////////////////////////////////////////////
                // var soRecrd = nlapiLoadRecord("salesorder", salesOrdId);
                // var soRate = soRecrd.getLineItemValue("item", "rate", i);
                // nlapiLogExecution("DEBUG", "soRecrd--------------- ", soRecrd);
                // nlapiLogExecution("DEBUG", "soRate--------------- ", soRate);
                ///////////////////////////////////////////////////////////////
                ...
                if (exclInPrint == "" || exclInPrint != "T") {
                    ...
                    template += "   <#if record.currency?string != 'AED'> <tr> <#else><tr style=\"border-bottom:solid 1px #000000;\"></#if>";
                    if (rental_unit == 5)
                    ...
                    else if (Item_type == "serviceitem") {
                        template += "<td align=\"right\" colspan=\"3\" style=\"border-left:solid 1px #000000;\">" + numberWithCommas(removeNull(robd_unit)) + "</td> ";
                        // template += "<td align=\"right\" colspan=\"3\" style=\"border-left:solid 1px #000000;\">" + numberWithCommas(removeNull(soRate)) + "</td> ";
                    }
                    else {
        ...
        template += "         <tr> " +
            ...
            "&nbsp;words: </b><i>${record.currency}&nbsp;&nbsp; ${record.custbody_amount_in_words_english_sar?capitalize}</i></td> " +
        ...
        template += "<td align=\"left\" colspan=\"22\" style=\"padding:5px;\"><span class=\"arabicfont\" dir=\"rtl\">${record.custbody_amount_in_words_arabic_sar} : المبلغ بالكلمات</span></td>";
        template += "</tr>";
        template += "     </table> ";
    ...
    if (sub_id == 11 || sub_id == 12) { //For Faris Al Arab Transport Company   <#else> <td style=\"width:336px;font-weight:bold;\" align=\"right\">For ${record.subsidiary.legalname} / <span class=\"arabicfont\" dir=\"rtl\">لشركة الفارس السعودية الدولية</span></td>
        ...
        template += "_________________________________ </td><#if record.custbody_seller_name_?has_content> <td style=\"width:336px;font-weight:bold;\" align=\"right\">For ${record.custbody_seller_name_} / <br/> <span class=\"arabicfont\" dir=\"rtl\">${record.subsidiary.custrecord_arabic_legalname} </span></td></#if> ";
