function printEnergyDivision(request, response) {
    ...
    if (headerlogo) {
        if(sub_id == 16){
            template += "<img class=\"header\" style=\"width:67%;height:38%;margin-left:-47px; margin-right:-65px;margin-top:-48px;\" height=\"60\%\" width=\"100\%\"  src = \"";
            var path = "https://7061228.app.netsuite.com" + nlapiLoadFile(headerlogo).getURL();
            template += nlapiEscapeXML(path);
            template += "\"></img>";
        }else{
            template += "<img class=\"header\" style=\"width:57%;height:38%;margin-left:-47px; margin-right:-65px;margin-top:-48px;\" height=\"60\%\" width=\"92\%\"  src = \"";
            var path = "https://7061228.app.netsuite.com" + nlapiLoadFile(headerlogo).getURL();
            template += nlapiEscapeXML(path);
            template += "\"></img>";
        }
    ...
    if (footerlogobottom) {
        template += "<img class=\"footer\" style=\"width:96%; height:100%;top:45px;margin-left:-50px;\" height=\"100\%\" width=\"96\%\"  src = \"";
        var path = "https://7061228.app.netsuite.com" + nlapiLoadFile(footerlogobottom).getURL();
        template += nlapiEscapeXML(path);
        template += "\"></img>";
    }
    if (footerlogotop) {
        template += "<img class=\"footer\" style=\"top:-50px;left:510px;margin-right:-40px;width:85%; height:100%;\" height=\"100\%\" width=\"88\%\"  src = \"";
    ...
    template += "</style> ";
    template += "   </head> ";
    template += "   <body header=\"nlheader\" header-height=\"12%\" footer=\"nlfooter\" footer-height=\"10%\" padding=\"0.5in 0.3in 0.3in 0.4in\" size=\"A4\"> ";
    template += "      <table style=\"width::50%;align: center;font-size:17px;top:10px;\"> ";
    template += "         <tr> ";
    template += '            <td style=\"font-weight:bold;\" align=\"center\">CREDIT NOTE # / <span class="arabicfont" dir="rtl" style=\"font-size: 17pt;\">اشعار دائن </span></td> ';
    //template += '            <td style=\"width:346px;font-weight:bold;\" align=\"right\">DATE &nbsp;&nbsp;<span class="arabicfont" dir="rtl">  تاريخ الفاتورة</span> : ${record.trandate?string["d-MMM-yyyy"]}</td> ';
    template += "         </tr> ";
    template += "      </table> ";
