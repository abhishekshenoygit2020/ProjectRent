function printQuoteEnergyFormatAction(request, response) {
    ...
    if (footerlogobottom) {
        // template += "<img class=\"footer\" style=\"width:96%; height:100%;top:60px;margin-left:-50px;\" height=\"100\%\" width=\"96\%\"  src = \"";
        // var path = "https://7061228.app.netsuite.com" + nlapiLoadFile(footerlogobottom).getURL();
        // template += nlapiEscapeXML(path);
        // template += "\"></img>";
    }
    if (footerlogotop) {
        template += "<p style=\"font-size: 8pt;top:35px;width: 75%\" align=\"left\">${record.subsidiary}/${record.tranid}<#if record.entity.isperson?string == 'T'>${record.entity.altname}<#else>${record.entity.companyname}</#if></p>";
    ...
    template += "    </macrolist> " +
        ...
        "</style> " +
        "</head> " +
        "  <body style=\"font-family:sans-serif;background-color:#ffffff;\" header=\"nlheader\" footer-height=\"6%\" header-height=\"9%\" footer=\"nlfooter\" background-image-width=\"8.5in\" background-image-position=\"bottom center\" background-image-height=\"4in\" padding=\"1.3in 0.5in 0.5in 0.5in\"   size=\"Letter\" > " +
        ...
        +
        "<#if record.subsidiary.internalid?string == '11' || record.subsidiary.internalid?string == '15' || record.subsidiary.internalid?string == '8'>" +
    ...
    template += "<hr /></#if> " +
        ...
        "      <#else> " +
        "        <tr> " +
        "      <td style=\"font-size:12px;\"><b>2. VAT </b>-3. VAT - Value added Tax of 15% shall be applicable for the above  full quoted price as per the KSA Federal law..</td> " +
        "      </tr> " +
      ...
      //${record.subsidiary.legalname}
        ...
        "  <tr><td style=\"width:50%;\"><b>For Saudi Al Faris International Company-Energy Division</b><br />(As \"Owner\")<br /><br /><br /><b>${record.salesrep.salutation} ${record.salesrep.firstname} ${record.salesrep.lastname}</b><br />${record.salesrep.title}<br />${record.salesrep.email}<br />${record.salesrep.mobilephone}</td> " +
