function printInvoiceAction(request, response) {
    ...
    template += "<?xml version=\"1.0\"?><!DOCTYPE pdf PUBLIC \"-//big.faceless.org//report\" \"report-1.1.dtd\"> " +
        ...
        "      <link name=\"arabic-font\" type=\"font\" subtype=\"opentype\" src=\"https://4647359.app.netsuite.com/core/media/media.nl?id=6857&c=4647359&h=GURyHDbOe5NQ6BZxduKx_Gh50v23P-0dnJKOlG4s0A4J4EOm&_xt=.ttf\" src-bold=\"https://4647359.app.netsuite.com/core/media/media.nl?id=21692&c=4647359&h=uHRS2GeV3S7MJFTJYAm2KCuCUsmbBvK_4BERfClx96I1yk1e&_xt=.ttf\" bytes=\"2\" />" +
    ...
    if (headerlogo) {
        ...
        // template += "\"></img>";
        template += "<p align=\"right\"  style=\"margin-top:50px;\"><b>&nbsp;</b></p>";
    ...
    template += "            <#if record.subsidiary.custrecord_is_energy?string == 'Yes'> " +
        ...
        "                           <td><b>Fax #</b> ${record.entity.fax}</td> " +
        "                        </tr> " +
        "                          <#if record.entity.internalid?string == '21410'||record.entity.internalid?string == '16102'||record.entity.internalid?string == '16103'||record.entity.internalid?string == '15961'> " +
        "                            <tr> " +
    ...
    if (footerlogotop) {
        if (subsidiary != 14) { // AFIHER AD
            // template += "<i style=\"font-size: 8pt;top:-30px;width: 75%\" align=\"left\">A.Any discrepancies must be notified to Accounts Department within 07 days from receipt of this invoice.<br/>B.Invoices should be settled as per approved credit terms. In case of delay, a surcharge of 1% per month, <br/>on overdue amount will be levied extra.</i>";
            template += "<p style=\"font-size: 8pt;top:-10px;width: 75%\"><i  align=\"left\">A.Any discrepancies must be notified to Accounts Department within 07 days from receipt of this invoice.<br/>B.Invoices should be settled as per approved credit terms. In case of delay, a surcharge of 1% per month, <br/>on overdue amount will be levied extra.</i></p>";
        } else {
            template += "<p style=\"font-size: 8pt;top:-10px;width: 75%\" align=\"left\">TRN : ${record.custbody_subsidiary_trn}<br/>${record.subsidiary}/" + Contract(quoteNo) + "<#if record.entity.isperson?string == 'T'>${record.entity.altname}<#else>${record.entity.companyname}</#if></p>";
        }
        // template += "<p style=\"font-size: 8pt;top:-5px;width: 75%\" align=\"left\">A.Any discreancies must be notified to Accoubts Department within 07 days from receipt of his invoice.</p>";
        // template += "<p style=\"font-size: 8pt;top:-10px;width: 75%\" align=\"left\">B.Invoices should be settled as per approved credit terms. In case of delay, a surcharge of 1% per month, on overdueamount will be levied extra.</p>";
        // template += "<p style=\"font-size: 8pt;top:-10px;width: 75%\" align=\"left\">TRN : ${record.custbody_subsidiary_trn}<br/>${record.subsidiary}/" + Contract(quoteNo) + "<#if record.entity.isperson?string == 'T'>${record.entity.altname}<#else>${record.entity.companyname}</#if></p>";
        template += "<p align=\"right\" style=\"top:-55px;font-size: 8pt;\">(Page <pagenumber/> of <totalpages/>)</p>";
        // template += "<img class=\"footer\" style=\"top:-81px;left:510px;margin-right:-40px;width:85%; height:100%;\" height=\"100\%\" width=\"88\%\"  src = \"";
    ...
    if ("T" == invoice.getFieldValue("custbody_allow_lump_sum")) {
        ...
        if (lineCount > 0) {
            for (var i = 1; i <= lineCount; i++) {
                ...
                if (exclInPrintamount > 0 || exclInPrint != "T") {
                    ...
                    if (item_type == "7") {
                        ...
                        template += "<tr>";
                        template += "<td colspan=\"3\" style=\"text-align:center;\">&nbsp;</td>";
            ...
            +
            "         <tr> " +
            "            <td align=\"left\" colspan=\"34\" style=\"padding:5px;border-top:solid 1px #000000;\"><b>Amount in" +
            ...
            for (var i = 1; i <= lineCount; i++) {
                ...
                if (exclInPrintamount > 0 || exclInPrint != "T") {
                    ...
                    if (extraLineRequired == 1 && itemType != 5 && itemType != 7 && Discount != "Discount") {
                        ...
                        if (billing_rule == "HOURLY") {
                            var rate = invoice.getLineItemValue('item', 'custcol_agreed_rate', i);
                            ...
                            if ((normalHours > 0 && PreHours > 0)) {
                                ...
                                nlapiLogExecution("DEBUG", "normalHours--1", normalHours);
                                nlapiLogExecution("DEBUG", "grossamt--1", grossamt);
                            ...
                            if (extra_hours > 0 && extra_hours_rate > 0) {
                                ...
                                var tax1amt = (+amount / 100) * parseFloat(taxrate1);
                                var grossamt = parseFloat(amount) + parseFloat(tax1amt);
                                nlapiLogExecution("DEBUG", "extra_hours--2", extra_hours);
                                nlapiLogExecution("DEBUG", "grossamt--2", grossamt);
                            ...
                            nlapiLogExecution("DEBUG", "not hourly---");

                            if ((normalHours > 0 && PreHours > 0) || itemType == 5 || itemType == 7 || Discount == "Discount") {
                                if (rental_unit == "Daily") {
                                    rental_unit = "Days"
                                }
                                if (rental_unit == "Monthly") {
                                ...
                                if (itemType == 7 || Discount == "Discount") {
    ...
    userID = userID['user'];
    if (userID == 20 || userID == 13 || userID == 42502 || userID == 3 || userID==84922) { //if Amount is miss-matching Even Then this role can See the Print 
