...
var field = nlapiGetField(name);
	...
	if(chkVal == true && name == 'undepfunds')
	{
		accountList(1);
	}
	if(chkVal == false && name == 'undepfunds'){
	...
	{
    // console.log(field.uifield.id);
		if(field.name == 'account' && field.uifield.id == "hddn_account4"){
...
function makeIJVAction(){	
	...
	if(pvjiv){
		...
		if(recordVal == "T"){		
			...
			var amount = nlapiGetFieldValue('payment');	// document.getElementById("payment_formattedValue").value;
			//var memo = nlapiGetFieldValue("memo");
			var custName = document.getElementById("customer_display").value;
			...
			rec.setLineItemValue('line', 'credit', 1, amount);
			...
			rec.setLineItemValue('line', 'debit', 2, amount);
			...
			rec.setLineItemValue('line', 'credit', 3, amount);
			...
			rec.setLineItemValue('line', 'debit', 4, amount);
